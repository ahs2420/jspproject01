...
;(function($, window, document, undefined) {
	...
	Owl.Workers = [ {
		...
		run: function() {
			var clones = [],
				...
				// TODO: Should be computed from number of min width items in
				// stage
	...
	Owl.prototype.initializeStage = function() {
		...
		// if the stage is already in the DOM, grab it and skip stage
		// initialization
	...
	/**
	 * @returns {Boolean} visibility of $element if you know the carousel will
	 *          always be visible you can set `checkVisibility` to `false` to
	 *          prevent the expensive browser layout forced reflow the
	 *          $element.is(':visible') does
	 */
			...
			'initialized.owl.carousel change.owl.carousel resized.owl.carousel': $.proxy(function(e) {
				...
				if ((e.property && e.property.name == 'position') || e.type == 'initialized') {
					...
					if (settings.lazyLoadEager > 0) {
						...
						// If the carousel is looping also preload images that
						// are to the "left"
						if (settings.loop) {
...
;(function($, window, document, undefined) {
	...
	Navigation.prototype.initialize = function() {
		...
		/*
		 * $el.on('focusin', function() { $(document).off(".carousel");
		 * 
		 * $(document).on('keydown.carousel', function(e) { if(e.keyCode == 37) {
		 * $el.trigger('prev.owl') } if(e.keyCode == 39) {
		 * $el.trigger('next.owl') } }); });
		 */
