...
<main class="resive-main">
    <form action="/article/modifyArticle" enctype="multipart/form-data" method="post" class="chkFormCke" id="uploadForm">
        
        <input type="hidden" name="fileOriName" value="${avo.fileOriName }"/>
        <input type="hidden" name="fileUrl" value="${avo.fileUrl }"/>
        <input type="hidden" name="fileName" value="${avo.fileName }"/>
          ...
          <div class="resive-container">
            <div class="resive-wrap">
                ...
                <div class="resive-border-box mt50">
                    ...
                    <button type="button">
                        <c:choose>
                            <c:when test="${avo.fileName ne ''}">
                                <img id="preview" class="gray-box" src="${avo.fileUrl}${avo.fileName}" data-cke-target="editor1" />
                            </c:when>
                            <c:otherwise>
                                <img id="preview" class="dis-none gray-box" src="${avo.fileUrl}${avo.fileName}" data-cke-target="editor1" />
                            </c:otherwise>
                        </c:choose>
                    </button>
                </div>
